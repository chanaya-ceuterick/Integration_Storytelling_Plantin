...
buttons.forEach((button) => {
  button.addEventListener("click", () => {
    const interactionElement = button
      .closest(".c-language__images")
      .querySelector(".c-language__click");
    const noninteractionElement = button
      .closest(".c-language__images")
      .querySelector(".c-language__imgone");
...
mm.add("(prefers-reduced-motion: reduce)", () => {
  ...
  gsap.to(".c-quote__txt", { opacity: 1, x: 0, duration: 0 });
...
mm.add("(prefers-reduced-motion: no-preference)", () => {
  ...
  mm.add("(min-width: 955px)", () => {
    // quote
    gsap.fromTo(
      "#quote-title-one, #quote-title-two, #quote-title-three, #quote-title-four, #star, #quote-one, #quote-two, #quote-three",
      {
        y: "120%",
        opacity: 0,
        rotation: 10,
  ...
  mm.add("(min-width: 769px)", () => {
    ...
    gsap.fromTo(
      ".c-intro__txt h2, .c-intro__txt p",
      {
        color: "transparent",
      },
      {
    ...
    //business women
    gsap.fromTo(
      ".c-women__worker, .c-women__speaking",
      {
        x: "-200%",
        rotation: -30,
      },
    ...
    options.forEach((option) => {
      option.addEventListener("dragstart", (e) => {
        ...
        options.forEach((opt) =>
          opt.classList.remove("c-women__option--active")
        );
      ...
      elements.forEach((element) => {
        ...
        textNodes.forEach((node) => {
          ...
          const wrappedWords = words
            .map((word) => `<span class="c-word">${word}</span>`)
            .join(" ");
